...
% Training the MNR model 
 while(true) 
    ...
    Num_trial = 0;
    % it is multiply of weight and Trainingsets
    H = X * W; 
    H_Max = max(H,[],2);
...
% cinsider Lambda = 0 and find MNR misclassification rate obtained for this
% Lambda
Error_H = [];
Htest = Xtest * W;
...
plot (test3,k);
xlabel('iteration', 'FontSize', 20, 'FontWeight', 'bold');
ylabel('RACE at each iteration', 'FontSize', 20, 'FontWeight', 'bold');
